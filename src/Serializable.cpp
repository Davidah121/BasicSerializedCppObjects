...
void SerializedObject::serialize(XmlNode* parent)
{
	...
	XmlNode* thisNode = new XmlNode();
	thisNode->title = "SerializedObject";
	...
	for(SerializedData* temp : variables)
	{
		...
		else
		{
			...
			thisNode->childNodes.push_back(nNode);
		}
	}

	if(parent != nullptr)
	{
		parent->childNodes.push_back(thisNode);
	}
}
...
void SerializedObject::deserialize(std::string s)
{
	...
	if(serData.nodes.size() > 0)
	{
		//Skip Serializer and grab first child.
		deserialize(serData.nodes[0]->childNodes[0]);
	}
}
...
void SerializedObject::deserialize(XmlNode* n)
{
	...
	for(XmlNode* cNodes : n->childNodes)
	{
		if(cNodes->title == "SerializedObject")
		{
			((SerializedObject*)variables[varNum])->deserialize(cNodes);
		}
		else if(cNodes->title == "SerializedData")
		{
			std::string typeString = cNodes->getAttribute("TYPE").second;
			std::string valueString = cNodes->getAttribute("VALUE").second;
