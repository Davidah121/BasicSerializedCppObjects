...
SimpleXml::SimpleXml(std::string f)
{
    load(f);
}
...
void SimpleXml::save(std::string file, bool includeTabs)
{
 ...
 */
bool SimpleXml::load(std::string file, bool parseEscape)
{
...
int SimpleXml::parseEscapeString(std::string escString)
{
    ...
    if(escString.front()=='&' && escString.back()==';')
    {
        ...
        if(num)
        {
            ...
            {
                //decimal number
                return StringTools::toInt( infoToParse );
            }
        }
        else
        {
